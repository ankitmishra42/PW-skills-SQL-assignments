...
SELECT name, attendance, 
	LEAD(attendance) OVER(ORDER BY attendance) AS next_stu_attendance
FROM Student_Performance;


-- Question 5 : Assign students into 4 performance groups using NTILE().
SELECT student_id, name, score,
	NTILE(4) OVER(ORDER BY score DESC) AS performance_groups
FROM Student_Performance;
...
SELECT name, CONCAT(MONTHNAME(join_date),' ', YEAR(join_date)) AS joins
FROM Student_Performance;
